...
function getMaxScore(nums) {
  const dp = []; // dp[i][k] = nums[i] までで k を作れるか否か
  const sum = nums.reduce((a, b) => a + b);
  for (let k = 0; k < sum; k++) dp[-1][k] = false;
  dp[-1][0] = true;
...
class Fin {
  ...
  draw() {
    ...
    this.game.scores.forEach((score, lv) => {
      ...
      // text("/ " + getMaxScore(getStageNums(lv)), x + P.fin.logMaxScoreX, y + P.fin.logMaxScoreY);
...
class Stage {
  ...
  addNumBall(item) {
    // console.log(item, item.vel.x);
    // if (!(item.vel.x < 10)) debugger;
